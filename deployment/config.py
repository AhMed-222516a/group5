...
class PathConfig:
    ...
    Attributes:
        ...
        model_dev_dir:       ``<root>/Model development/``
        outputs_dir:         ``<root>/Model development/outputs/``
        cache_dir:           ``<root>/deployment/cache/``
        logs_dir:            ``<root>/deployment/logs/``
        temp_dir:            ``<root>/deployment/temp/``
        models_dir:          ``<root>/deployment/models/``
        faiss_dir:           ``<root>/deployment/models/faiss/``
    ...
    def build(cls) -> "PathConfig":
        ...
        raw = data / "raw"
        graphs = root / "graphs"
        model_dev = root / "Model development"
        outputs = model_dev / "outputs"
