...
def build_prompt(query: str, retrieval_results: List[RetrievalResult]) -> str:
    """Construct a compact grounded prompt for the generation model.

    Optimised for Flan-T5's 512-token input limit. Uses compact context
    format and omits the few-shot example to save tokens.
    ...
    retrieval_results:
        Ranked retrieval results used to build the context block.

    Returns
    -------
    str
        A compact prompt ready for the generation pipeline.
    """
    parts: List[str] = []

    parts.append(
        "Answer the customer support question using ONLY the retrieved "
        "tickets below. Be concise, professional, and specific. If the "
        "tickets lack sufficient information, say exactly: "
        '"I could not find sufficient information in the retrieved tickets."'
    )
    ...
    if retrieval_results:
        ...
        for r in retrieval_results:
            context_lines.append(
                f"Ticket {r.rank}: {r.subject} | {r.category}/{r.priority} | "
                f"Resolution: {r.resolution_note}"
            )
        parts.append("\n".join(context_lines))
    else:
        parts.append("Context: No relevant tickets found.")
